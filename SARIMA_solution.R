...
AP_dummy <- seasonaldummy( AP )
AP_data <- data.frame( cbind( AP, AP_dummy ) )
AP_data$trend <- c( 1:length( AP_data$AP ) )
lm_decomp <- lm( AP ~., data = AP_data )
plot( lm_decomp$fitted.values, type = "o", pch = 20, col = "red" ) +
  lines( AP_data$AP )
summary( lm_decomp )

AP_decompM <- decompose( AP, type = "multiplicative" )
plot( AP_decompM )
...
par( mfrow = c( 2, 1 ) )
acf( AP_diff, main = "ACF" )
pacf( AP_diff, main = "PACF" )
dev.off()
